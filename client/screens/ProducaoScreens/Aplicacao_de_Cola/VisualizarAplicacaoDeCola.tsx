...
import { getPneuAplicacaoDeCola } from "../../../services/pneuCRUD";
import { RootStackParamList, PneuItem } from "../../../src/types";

// Definindo o tipo de navegação com base no RootStackParamList
type VisualizarAplicacaoDeColaNavigationProp = StackNavigationProp<
  RootStackParamList,
  "AplicacaoDeColaScreen"
>;

type Props = {
  navigation: VisualizarAplicacaoDeColaNavigationProp;
};

const VisualizarAplicacaoDeCola: React.FC<Props> = ({ navigation }) => {
  const [pneuData, setPneuData] = useState<PneuItem[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      const { data, error } = await getPneuAplicacaoDeCola();
  ...
  const renderTableRow = ({ item }: { item: PneuItem }) => (
    <View style={styles.tableRow}>
      ...
      <TouchableOpacity
        ...
        onPress={() =>
          navigation.navigate("AplicacaoDeColaScreen", { tireId: item.ID_Pneu.toString() })
        }
      >
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Pneus - AplicacaoDeCola</Text>
...
export default VisualizarAplicacaoDeCola;
